...
class RobotArm {
  ...
  // Inverse Kinematics
  set x(v) {
    this.position.x = v - this.origin.x;
  }
  set y(v) {
    this.position.y = v - this.origin.y;
  }
  get a1() {
    return this.upperArm.len;
  }
  get a2() {
    return this.lowerArm.len;
  }
  get q1() {
    return (
      atan(this.position.y / this.position.x) - atan((this.a2 * sin(this.q2)) / (this.a1 + this.a2 * cos(this.q2)))
    );
  }
  get q2() {
    return acos((this.position.x ** 2 + this.position.y ** 2 - this.a1 ** 2 - this.a2 ** 2) / (2 * this.a1 * this.a2));
  }
...
class Arm {
  ...
  get θ() {
    return this.angleOfs + this.angle;
  }
  ...
  draw = () => {
    ...
    quad(lowerArmWidth, 0, -lowerArmWidth, 0, -upperArmWidth, this.len, upperArmWidth, this.len);
