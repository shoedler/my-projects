const isOutsideBoundaries = (b) => (b.position.x > width || b.position.x < 0) || (b.position.y > height || b.position.y < 0);
...
class Flock
{
  ...
  constructor(amount)
  {
    ...
    this.boidSize = 5; this.setPopulationProperty('boidSize');
    this.maxVelocity = 2; this.setPopulationProperty('maxVelocity');
    this.fieldOfView = 300; this.setPopulationProperty('fieldOfView');

    this.alignmentRadius = 50.0; this.setPopulationProperty('alignmentRadius');
    this.cohesionRadius = 70.0; this.setPopulationProperty('cohesionRadius');
    this.separationRadius = 20.0; this.setPopulationProperty('separationRadius');
    
    this.maxForceAlignment = 0.005; this.setPopulationProperty('maxForceAlignment');
    this.maxForceCohesion = 0.003; this.setPopulationProperty('maxForceCohesion');
    this.maxForceSeparation = 0.009; this.setPopulationProperty('maxForceSeparation');

    this.weightSeparation = 1; this.setPopulationProperty('weightSeparation');
    this.weightAlignment = 1; this.setPopulationProperty('weightAlignment');
    this.weightCohesion = 1; this.setPopulationProperty('weightCohesion');

    this.showAlignmentVector = false; this.setPopulationProperty('showAlignmentVector');
    this.showAlignmentFieldOfView = false; this.setPopulationProperty('showAlignmentFieldOfView');
    this.showCohesionVector = false; this.setPopulationProperty('showCohesionVector');
    this.showCohesionFieldOfView = false; this.setPopulationProperty('showCohesionFieldOfView');
    this.showSeparationVector = false; this.setPopulationProperty('showSeparationVector');
    this.showSeparationFieldOfView = false; this.setPopulationProperty('showSeparationFieldOfView');
    this.showFieldOfView = false; this.setPopulationProperty('showFieldOfView');
  }
   ...
   * @memberof Flock
   */
  setPopulationProperty = (prop) => {this.boids.forEach(b => {b[prop] = this[prop]})}
   ...
   * @memberof Flock
   */
  push = (amount) => { for (let i = 0; i < amount; i++) { this.boids.push(new Boid(random(0, width), random(0, height), i)); }; }
  ...
  * @memberof Flock
  */
  pop = (amount) =>  { for (let i = 0; i < amount; i++) { this.boids.pop(); }; }
  ...
  */
        ...
        { 
          ...
          if (d < current.alignmentRadius)
          {
            ...
            if (angle <= 180 && angle >= 180 - current.fieldOfView / 2)
            {
              ...
              if (current.showAlignmentVector) visualize(current.position, p5.Vector.sub(other.position, current.position), color(200, 100, 0), 1, 1);
          ...
          if (d < current.cohesionRadius)
          {
            ...
            {
              /* Debug: Visualize distance to detected neighbor */
              if (current.showCohesionVector) visualize(current.position, p5.Vector.sub(other.position, current.position), color(100, 0, 100), 1, 1);
  ...
  evalCohesion = (current, neighborSum, neighborCount) => 
  {
    if (neighborCount > 0)
    {
      ...
      if (current.showCohesionVector) { visualize(current.position, cohesionCorrection, color(0, 0, 155), 1, 50); }
   ...
   * @memberof Flock
   */
  bounds = () => { return this.boids.every(isOutsideBoundaries); }
}