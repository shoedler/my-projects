...
function setup()
{
    ...
    {
        aSoll.push(new UInt16(testValues[i]));
...
function draw()
{
    ...
    if (testIndex == bitAmount)
    {
        ...
        for (let i = 0; i < aIst.length; i++) 
        {
            if (aIst[i].value != 0)
            {
                console.log(`Error at test index ${i}: it's ${aIst[i].value} instead of ${testValues[i]}`)
                testIndex = i;
...
let simulate = () =>
{
    wIn.value = testValues[testIndex];

    // Add Error Value on index 12
    if (testIndex == 12) wIn.value = testValues[12] - 0x8;
}
